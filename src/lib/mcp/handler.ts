...
/**
 * The door an outside AI assistant knocks on, minus the route it is mounted on.
 *
 ...
 *
 * Shared by both mountings: /api/mcp, where the key arrives in an Authorization header, and
 * /api/mcp/<key>, where it is part of the address because claude.ai's connector dialog accepts
 * nothing else. Same handler, same checks; only where the credential was read differs.
 *
...
async function authorize(request: Request, keyFromPath?: string) {
  // Two ways in, because the clients differ. Claude Desktop and Claude Code send a header;
  // claude.ai's connector dialog takes a URL and nothing else, so the key rides in the path —
  // the same fallback most public MCP servers offer. A key in a URL is a weaker secret (it lands
  // in server logs and browser history), which is why the header is preferred wherever it works
  // and why the settings screen hands out a fresh key rather than reusing one.
  const header = request.headers.get("authorization") || "";
  const presented = keyFromPath?.trim() || (header.startsWith("Bearer ") ? header.slice(7).trim() : "");
  if (!presented) {
    return { ok: false as const, response: unauthorized("Missing key. Send it as Authorization: Bearer <key>, or put it in the URL.") };
  }

  const key = await verifyMcpKey(presented);
  if (!key) {
...
export async function handleMcpPost(request: Request, keyFromPath?: string) {
  const auth = await authorize(request, keyFromPath);
