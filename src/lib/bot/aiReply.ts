 ...
 *
 * Named and hoisted because POSITION in the prompt decides whether a model obeys it, not wording.
 * Measured 2026-09-07 over a 36-exchange battery: buried two-thirds down HARD_RULES, Gemini 3.1
 * Flash-Lite answered English messages in Arabic on 3 of 6 turns; repeated as the first line of
 * the system prompt it obeyed 6 of 6, then 4 of 4 on a full re-run. `gemini-flash-latest` obeyed
 * from either position, so the hoist costs the larger model nothing and is what makes a cheaper
 * one usable. Reproduce with `node scripts/probe-model-battery.mjs`.
 */
const LANGUAGE_RULE =
