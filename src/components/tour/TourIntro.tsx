 ...
 * Meet Sara.
 *
 * The first thing a new person sees after the setup wizard: a black screen, her orb, her name,
 * and one offer — to be shown the whole system. It appears once per person per clinic and never
 * by itself again; declining is one click and the tour stays a menu item away.
 *
...
export default function TourIntro() {
  ...
  return (
    <div
      ...
      aria-label={guide}
    >
      {/* One soft shape behind her, part of the composition rather than decoration. */}
      ...
      <div className="relative flex w-full max-w-lg flex-col items-center text-center animate-in fade-in slide-in-from-bottom-6 duration-700">
        ...
        <p className="mt-5 max-w-md text-[15px] font-medium leading-relaxed text-white/70">
          {isAr
            ? `هاخدك في جولة على ${clinic?.name || "النظام"} كله — كل شاشة، وكل إعداد، وكل مفتاح — وتقدر تسألني أي حاجة في أي وقت. لو زهقت، اقفل وارجع من نفس المكان بعدين.`
            : `I'll show you the whole of ${clinic?.name || "the system"} — every screen, every setting, every switch — and you can ask me anything along the way. Stop whenever you like and pick up where you left off.`}
        </p>

        <p className="mt-4 flex items-center gap-1.5 text-[12px] font-bold text-white/40">
          <Clock size={13} />
          {isAr ? `حوالي ${minutes} دقيقة · ${tour.stops.length} محطة` : `About ${minutes} minutes · ${tour.stops.length} stops`}
        </p>
