...
export default function PatientTimelineTab({ patientId }: { patientId: string }) {
  ...
  if (visits.length === 0) {
    return (
      <div className="text-center py-20 bg-surface rounded-3xl border border-slate-100 shadow-sm">
        <CalendarDays size={64} className="mx-auto text-slate-200 mb-4" />
        <p className="text-ink-muted font-semibold text-lg">{language === "ar" ? "لا توجد زيارات سابقة" : "No visits found"}</p>
        ...
        {visits.map((visit, index) => {
          ...
          return (
            <div key={visit.id} className="relative group">
              ...
              <div className="bg-surface rounded-3xl p-5 border border-line shadow-sm hover:shadow-md transition-all group-hover:border-primary-200">
                
                {/* Header */}
                <div className="flex flex-wrap items-start justify-between gap-4 mb-5">
                  <div>
                    <h3 className="text-lg font-bold text-slate-800">{title}</h3>
                    <p className="text-ink-muted text-sm flex items-center gap-2 mt-1 font-medium">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    <div className="flex items-center gap-3">
                      {isAppt && (
                        <span className="px-3 py-1 bg-surface-muted text-slate-700 text-xs font-bold rounded-full border border-line">
                          {visit.status}
                        </span>
                      )}
                      <button 
                ...
                {isAppt ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-surface-subtle rounded-2xl p-4 border border-slate-100">
                      ...
                      {visit.statusHistory && visit.statusHistory.length > 0 ? (
                          ...
                          {visit.statusHistory.map((h, i) => {
                            ...
                            return (
                              <div key={i} className="flex gap-3 relative z-10 text-sm">
                                <div className="w-4 h-4 rounded-full bg-surface border-2 border-line-strong shrink-0 mt-0.5"></div>
                                <div>
                        ...
                        <div className="text-sm text-ink-muted italic bg-surface p-3 rounded-xl border border-slate-100">
                    ...
                    <div className="bg-surface-subtle rounded-2xl p-4 border border-slate-100">
                      ...
                      {visit.noteDetails ? (
                        <div className="bg-surface p-3.5 rounded-xl border border-line shadow-sm">
                          <p className="font-semibold text-sm text-slate-800">{visit.noteDetails.procedure || visit.noteDetails.title || "Note"}</p>
                          <p className="text-xs font-bold text-ink-muted mt-2 flex items-center gap-1.5">
                          ...
                          {visit.noteDetails.note && (
                            <div className="mt-3 bg-surface-subtle p-2.5 rounded-lg border border-slate-100">
                               <p className="text-sm text-ink-body italic">"{visit.noteDetails.note}"</p>
                            </div>
                          )}
                        </div>
                      ) : (
                  ...
                  <div className="bg-emerald-50/50 rounded-2xl p-4 border border-emerald-100/60">
                    ...
                    <div className="bg-surface p-4 rounded-xl border border-emerald-100 shadow-sm">
                      ...
                      {visit.note && (
                        <div className="mt-4 bg-surface-subtle p-3 rounded-lg border border-slate-100">
                           <p className="text-sm text-ink-body italic">"{visit.note}"</p>
                        </div>
                      )}
                    </div>
                  </div>
