...
import {
  Send, Check, Loader2, Inbox, Clock, ExternalLink, RotateCcw,
} from "lucide-react";
 ...
 *
 * A tab of the Intelligence page rather than a page of its own: these messages are the system's
 * output waiting on a human, so they belong beside the brief that explains why they were written.
 */
...
export default function MessageQueuePanel() {
  ...
  return (
    <div className="space-y-4">
      {/*
        Patients the assistant handed to a person, above the send queue: the queue is messages
        the clinic wants to send, this is people waiting on the clinic.
      */}
      <HandoffInbox />
      ...
      {!showSent && visible.length > 0 && (
        <p className="text-xs font-bold text-emerald-800 bg-emerald-50 border border-emerald-100 rounded-2xl px-4 py-3 leading-relaxed">
          {isAr
