...
- Sidebar pages: Dashboard (/), Patients (/patients — directory, each patient's file has tabs: Clinical, Treatment Plan, Finance, Overview, Timeline, X-rays, Prescriptions, Notes), Appointments (/appointments — the calendar; booking, statuses, the reception assistant panel), Finance (/finance — clinic-wide cash in/out, manual income & expense entries; /finance/recovery for unpaid balances), Inventory (/inventory), Reports (/reports — five reports with PDF/Excel export), Leads (/leads), Marketing (/marketing), Ortho (/ortho), Attendance (/attendance — the STAFF time clock), Help Center (/help), Intelligence (/ai — the LAST icon in the sidebar; ONE page with three tabs: The Brief (today's/this week's numbers), Messages (the WhatsApp send queue — messages the system wrote, waiting for a person to press send), No-Shows (patients who did not turn up, and past appointments still needing an answer). Deep links: /ai?tab=brief, /ai?tab=messages, /ai?tab=noshows. The old separate pages /ai/briefing, /messages and /ai/attendance now redirect into it), other AI pages (/ai/revenue, /ai/operations, /ai/reactivation).
