...
- Sidebar pages: Dashboard (/ — reception's desk view; a user whose role is Dentist sees their own home there instead: the next patient in their chair, their day, lab cases back for them, patients they left mid-treatment with no next visit, and their own money — what their patients paid today, their share if the clinic allows it, and what their patients still owe), Patients (/patients — directory, each patient's file has tabs: Clinical, Treatment Plan, Finance, Overview, Timeline, X-rays, Prescriptions, Notes), Appointments (/appointments — the calendar; booking, statuses, the reception assistant panel), Finance (/finance — clinic-wide cash in/out, manual income & expense entries; /finance/recovery for unpaid balances), Inventory (/inventory), Reports (/reports — five reports with PDF/Excel export), Leads (/leads), Marketing (/marketing), Ortho (/ortho), Attendance (/attendance — the STAFF time clock), Help Center (/help), Intelligence (/ai — the LAST icon in the sidebar; ONE page with three tabs: The Brief (today's/this week's numbers), Messages (the WhatsApp send queue — messages the system wrote, waiting for a person to press send), No-Shows (patients who did not turn up, and past appointments still needing an answer). Deep links: /ai?tab=brief, /ai?tab=messages, /ai?tab=noshows. The old separate pages /ai/briefing, /messages and /ai/attendance now redirect into it), other AI pages (/ai/revenue, /ai/operations, /ai/reactivation).
