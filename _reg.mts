...
async function cleanup(FROM: string) {
  ...
  for (const ph of ["+" + FROM, FROM]) {
    for (const c of ["leads", "patients"]) {
      const snap = await clinic.collection(c).where("phone", "==", ph).get();
      for (const d of snap.docs) {
        if (c === "patients") {
          const ap = await clinic.collection("appointments").where("patientId", "==", d.id).get();
          for (const a of ap.docs) await a.ref.delete();
        }
        await d.ref.delete();
      }
    }
...
async function send(FROM: string, text: string) {
  const convRef = clinic.collection("whatsapp_conversations").doc(conversationKey("+" + FROM));
  const t0 = Date.now();
  const body = { object: "whatsapp_business_account", entry: [{ id: "1392032743138930", changes: [{ value: { messaging_product: "whatsapp", metadata: { phone_number_id: "1314232971773536" }, messages: [{ from: FROM, id: `wamid.r${Date.now()}${Math.floor(t0 % 997)}`, type: "text", text: { body: text } }] }, field: "messages" }] }] };
...
const EN = "201000000791";
const AR = "201000000792";
try {
  await cleanup(EN);
  console.log("=== ENGLISH patient: the whole booking flow must be English ===");
  await send(EN, "Hi, I want to book a cleaning");
  await send(EN, "ok");
  await send(EN, "John Regression");
  await cleanup(EN);

  await cleanup(AR);
  console.log("\n=== ARABIC patient who taps a button: must stay Arabic ===");
  await send(AR, "عايز احجز");
  await send(AR, "m1");
  await cleanup(AR);
} finally {
  await cleanup(EN);
